package medizin.client.ui.widget.widgetsnewcustomsuggestbox.test.client.ui.widget.suggest;
...
import medizin.client.ui.widget.widgetsnewcustomsuggestbox.test.client.ui.widget.suggest.impl.DefaultSuggestionPopup;
...
import medizin.client.ui.widget.widgetsnewcustomsuggestbox.test.client.ui.widget.suggest.impl.simple.DefaultValueRendererFactory;
import medizin.client.ui.widget.widgetsnewcustomsuggestbox.test.client.ui.widget.suggest.param.Option;
...
public abstract class AbstractSuggestBox<T, W extends EventHandlingValueHolderItem<T>>
	...
	// protected StringFormulator<T> stringFormulator = new
	// DefaultStringFormulator<T>();
	...
	protected void init(String defaultText) {
		...
		scrollPanel.addHandler(new MouseOutHandler() {
					...
					if (keyCode == KeyCodes.KEY_ENTER) {
						if (isShowingSuggestList()) {
							...
							if (multipleChangeEvent) {
								// popup is not visible, this enter means OK
								// (check if the
								// value was entered from the list or from
								// text).
								if (selected == null)
									valueTyped(getText());
								else
						...
						recomputePopupContent = true;
						if (strictMode) {
							final StringBuffer reducingText = new StringBuffer(
									getText());
							recomputePopupContent(keyCode,
									new SuggestPossibilitiesCallBack<T>() {
										...
										public void setPossibilities(
											...
											if (reducingText.length() > 1
													&& possibilities.size() < 1) {
												// FIXME should optimize by
												// remembering the last valid
												// entry
												// (that has at least one
												// possibility)
												reducingText
														.setLength(reducingText
																.length() - 1);
	...
	protected void recomputeAllPopupContent(final int keyCode,
			final SuggestPossibilitiesCallBack<T> callBack) {
	...
	protected void showSuggestList() {
		suggestWidget.adjustPosition(getTextField().getAbsoluteLeft(),
				getTextField().getAbsoluteTop()
						+ getTextField().getOffsetHeight());
	...
	public void setRendererWidth(String width) {
		rendererWidth = width;
		/*
		 * System.out.println("setRendererWidth : " + width); if(listRenderer !=
		 * null) { System.out.println("Renderer is not null : " +
		 * listRenderer.getWidgetCount() ); int count =
		 * listRenderer.getWidgetCount(); for(int index=0;index<count;index++) {
	...
	public void computeSelected(String text) {
		computeFiltredPossibilities(text,
				new SuggestPossibilitiesCallBack<T>() {
	...
	/*
	 * public String getDefaultText() { return getTextField().getDefaultText();
	 * }
	 */
	...
	/*
	 * public Validator<String> getValidator() { return
	 * getTextField().getValidator(); }
	 * 
	...
	/*
	 * public StringFormulator<T> getStringFormulator() { return
	 * stringFormulator; }
	 */
