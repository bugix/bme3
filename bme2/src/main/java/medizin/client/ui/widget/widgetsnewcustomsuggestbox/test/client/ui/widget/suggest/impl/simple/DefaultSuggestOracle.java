...
import medizin.client.ui.widget.widgetsnewcustomsuggestbox.test.client.ui.widget.suggest.SuggestOracle.Response;
...
public class DefaultSuggestOracle<T> extends SuggestOracle<T> {
	...
	protected boolean accept(String text, T t) {
		...
		// options);
		// boolean caseSensitive = BooleanOption.isEnabled("CASE_SENSITIVE",
		// options);
		boolean caseSensitive = false;
		String stringFormula = suggestBox.toString(t);
		String stringValue = caseSensitive ? stringFormula : stringFormula
				.toUpperCase();
		String textValue = caseSensitive ? text : text.toUpperCase();

		// boolean startsWith =
		// BooleanOption.isEnabled(DefaultOptions.STARTS_WITH.name(), options);
