...
import medizin.client.ui.widget.widgetsnewcustomsuggestbox.test.client.ui.widget.suggest.ValueRendererFactory;
...
public class DefaultValueRendererFactory<T, W extends EventHandlingValueHolderItem<T>>
	...
	public W createValueRenderer(T value, String filterText,
		...
		// BooleanOption.isEnabled(DefaultOptions.CASE_SENSITIVE.name(),
		// options), this);
		// W toReturn = (W) new DefaultValueRenderer<T>(value, filterText,
		// BooleanOption.isEnabled("CASE_SENSITIVE", options), this);
		W toReturn = (W) new DefaultValueRenderer<T>(value, filterText, false,
				this);
	...
	@Override
	public W createValueRenderer(T value, String filterText,
			Map<String, Option<?>> options, Renderer<T> renderer) {
		// Window.alert(DefaultOptions.CASE_SENSITIVE.name());
		// W toReturn = (W) new DefaultValueRenderer<T>(value, filterText,
		// BooleanOption.isEnabled(DefaultOptions.CASE_SENSITIVE.name(),
		// options), this,renderer);
		// W toReturn = (W) new DefaultValueRenderer<T>(value, filterText,
		// BooleanOption.isEnabled("CASE_SENSITIVE", options), this,renderer);
		W toReturn = (W) new DefaultValueRenderer<T>(value, filterText, false,
				this, renderer);
		toReturn.setStyleName(EventHandlingValueHolderItem.ITEM_DEFAULT_STYLE);
		return toReturn;
